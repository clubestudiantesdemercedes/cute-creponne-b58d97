...
export async function resolvePrice(
  ...
  if (conditionType === 'convenio' && conventionId) {
    const [specific] = await db
      .select()
      .from(prices)
      .where(and(eq(prices.planId, planId), eq(prices.conditionType, 'convenio'), eq(prices.conventionId, conventionId)))
    if (specific) return specific.amount
  }
  const [generic] = await db
    .select()
    .from(prices)
    .where(and(eq(prices.planId, planId), eq(prices.conditionType, conditionType), isNull(prices.conventionId)))
