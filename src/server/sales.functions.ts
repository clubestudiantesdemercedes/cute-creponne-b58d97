...
import { db } from '../../db'
import { sales, saleItems, payments, permits, plans, people, auditLogs } from '../../db/schema'
import { requireUser } from './auth.server'
import { computePermitDates } from '@/lib/permit'
import { randomCode, generateSaleNumber } from '@/lib/permit'
import { todayISO } from '@/lib/format'
import { resolvePrice } from './plans.functions'
...
export const createSale = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    const insertedItems = await db
      .insert(saleItems)
      .values(resolvedItems.map((i) => ({ ...i, saleId: sale.id })))
      .returning()
    ...
    for (const item of insertedItems) {
      const plan = planById.get(item.planId)!
      const { startDate, endDate } = computePermitDates(plan, purchaseDate)
      permitRows.push({
        code: randomCode(),
        personId: item.personId,
        saleItemId: item.id,
    ...
    await db.insert(auditLogs).values({
      ...
      details: { total, itemCount: insertedItems.length, paymentMethod: data.paymentMethod },
    })

    const personIds = [...new Set(insertedItems.map((i) => i.personId))]
...
export const listSales = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data }) => {
    await requireUser()
    const conditions = []
    if (data.dateFrom) conditions.push(gte(sales.createdAt, new Date(data.dateFrom + 'T00:00:00')))
    if (data.dateTo) conditions.push(lte(sales.createdAt, new Date(data.dateTo + 'T23:59:59')))
    const rows = await db
      .select()
      .from(sales)
...
export const getSaleDetail = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data }) => {
    ...
    const items = await db
      .select({ item: saleItems, person: people, plan: plans, permit: permits })
      .from(saleItems)
      .innerJoin(people, eq(saleItems.personId, people.id))
      .innerJoin(plans, eq(saleItems.planId, plans.id))
      .leftJoin(permits, eq(permits.saleItemId, saleItems.id))
...
export const voidSale = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    const user = await requireUser()
    if (user.role !== 'admin') throw new Error('Solo un administrador puede anular ventas.')
    await db.update(sales).set({ status: 'anulada' }).where(eq(sales.id, data.saleId))
    const itemRows = await db.select({ id: saleItems.id }).from(saleItems).where(eq(saleItems.saleId, data.saleId))
    const itemIds = itemRows.map((r) => r.id)
    if (itemIds.length) {
      await db.update(permits).set({ status: 'anulado' }).where(inArray(permits.saleItemId, itemIds))
    }
    await db.insert(auditLogs).values({
      userId: user.userId,
      action: 'anular_venta',
